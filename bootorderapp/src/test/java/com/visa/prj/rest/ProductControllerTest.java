...
public class ProductControllerTest {
	...
	public void testGetProducts() throws Exception {
		...
		// @formatter:off
		mockMvc.perform(get("/products")).andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(2)))
				.andExpect(jsonPath("$[0].id", is(1))).andExpect(jsonPath("$[0].name", is("a")))
				.andExpect(jsonPath("$[1].id", is(2))).andExpect(jsonPath("$[1].name", is("b")));
		// @formatter:on
		verify(orderService, times(1)).getAllProducts(); //verify getAllProducts() is called just once
